...
class Energy extends Base
{
    ...
    public function setupPlannedConnections(array $plan, array $importedGeometryId): array|bool
    {
        //Import energy connections and output now we now all geometry is known by the importer.
        $geometry = new Geometry();
        $geometry->setGameSessionId($this->getGameSessionId());
        foreach ($plan['layers'] as $layer) {
            ...
            foreach ($layer['geometry'] as $layerGeometry) {
                ...
                $newGeometryId = $geometry->fixupGeometryID($layerGeometry['base_geometry_info'], $importedGeometryId);
                if (is_string($newGeometryId)) {
                    $return[] = $newGeometryId;
                    continue;
                }
                if (!empty($layerGeometry['cable'])) {
                    $startId = $geometry->fixupGeometryID($layerGeometry['cable']['start'], $importedGeometryId);
                    $endId = $geometry->fixupGeometryID($layerGeometry['cable']['end'], $importedGeometryId);
                    ...
                    if (is_int($startId) && is_int($endId) && $startId != -1 && $endId != -1) {
                        $this->insertRowIntoTable(
                            'energy_connection',
                            [
                                'energy_connection_start_id' => $startId,
                                'energy_connection_end_id' => $endId,
                                'energy_connection_cable_id' => $newGeometryId,
                                'energy_connection_start_coordinates' => $layerGeometry['cable']['coordinates'],
                                'energy_connection_lastupdate' => 100
                            ]
                        );
                    }
                }
                //Import energy output
                if (!empty($layerGeometry['energy_output'])) {
                    foreach ($layerGeometry['energy_output'] as $output) {
                        $this->insertRowIntoTable(
                            'energy_output',
                            [
                                'energy_output_geometry_id' => $newGeometryId,
                                'energy_output_maxcapacity' => $output['maxcapacity'],
                                'energy_output_active' => $output['active']
                            ]
                        );
                    }
                }
    ...
    public function setupPlannedGrids(array $plannedGrids, int $planId, array $importedGeometryId): array|bool
    {
        $geometry = new Geometry();
        $geometry->setGameSessionId($this->getGameSessionId());
        foreach ($plannedGrids as $grid) {
            $gridId = $this->insertRowIntoTable(
                'grid',
                [
                    'grid_name' => $grid['name'],
                    'grid_lastupdate' => 100,
            ...
            if ($grid['grid_persistent'] == $grid['grid_id']) {
                ...
                if (isset($importedGridIds[$grid['grid_persistent']])) {
                    ...
                    $return[] = "Found reference persistent Grid ID (". $grid['grid_persistent'].
                        ") which has not been imported by the plans importer (yet).";
                }
            }
            $this->updateRowInTable('grid', ['grid_persistent' => $gridPersistent], ['grid_id' => $gridId]);
            foreach ($grid['energy'] as $energy) {
                $this->insertRowIntoTable(
                    'grid_energy',
                    [
                        'grid_energy_grid_id' => $gridId,
                        'grid_energy_country_id' => $energy['country'],
                        'grid_energy_expected' => $energy['expected']
                    ]
                );
            }
            foreach ($grid['removed'] as $removed) {
                if (!empty($importedGridIds[$removed['grid_persistent']])) {
                    $this->insertRowIntoTable(
                        'grid_removed',
                        [
                            'grid_removed_plan_id' => $planId,
                            'grid_removed_grid_persistent' => $importedGridIds[$removed['grid_persistent']]
                        ]
                    );
                } else {
                    $return[] = "Found deleted Grid ID (". $removed['grid_persistent'].
                        ") which has not been imported by the plans importer (yet).";
                }
            }
            if (!empty($grid['sockets'])) {
                foreach ($grid['sockets'] as $socket) {
                    $geometryId = $geometry->fixupGeometryID($socket['geometry'], $importedGeometryId);
                    if (is_string($geometryId)) {
                        $return[] = $geometryId;
                        continue;
                    }
                    if ($geometryId != -1) {
                        $this->insertRowIntoTable(
                            'grid_socket',
                            [
                                'grid_socket_grid_id' => $gridId,
                                'grid_socket_geometry_id' => $geometryId
                            ]
                        );
                    }
                }
            ...
            if (!empty($grid['sources'])) {
                foreach ($grid['sources'] as $source) {
                    $geometryId = $geometry->fixupGeometryID($source['geometry'], $importedGeometryId);
                    if (is_string($geometryId)) {
                        $return[] = $geometryId;
                        continue;
                    }
                    if ($geometryId != -1) {
                        $this->insertRowIntoTable(
                            'grid_source',
                            [
                                'grid_source_grid_id' => $gridId,
                                'grid_source_geometry_id' => $geometryId
                            ]
                        );
                    }
                }
