...
CREATE TABLE IF NOT EXISTS `msp`.`plan` (
  ...
  `plan_time` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `plan_gametime` INT(5) NULL,
  `plan_state` ENUM('DESIGN', 'CONSULTATION', 'APPROVAL', 'APPROVED', 'IMPLEMENTED', 'DELETED') NULL DEFAULT 'DESIGN',
  `plan_lock_user_id` INT NULL,
  `plan_lastupdate` DOUBLE NULL,
  `plan_previousstate` ENUM('NONE', 'DESIGN', 'CONSULTATION', 'APPROVAL', 'APPROVED') NULL DEFAULT 'NONE',
  `plan_active` TINYINT NULL DEFAULT 1,
  `plan_constructionstart` INT NULL,
  `plan_type` VARCHAR(75) NULL COMMENT 'If a plan is energy/ecology/shipping. Comma separated value',
  `plan_energy_error` TINYINT(1) NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `msp`.`energy_output` (
  ...
  INDEX `fk_energy_output_geometry1_idx` (`energy_output_geometry_id` ASC),
