...
class Warning extends Base {
	...
	public function Post(array $added, array $removed)
	{
		foreach($added as $addedIssue) {
			$existingIssue = $this->query("SELECT warning_id FROM warning WHERE warning_active = 1 AND warning_layer_id = ? AND warning_issue_type = ? AND abs(warning_x - ?) <= 1e-6 AND abs(warning_y - ?) <= 1e-6 AND warning_source_plan_id = ? AND warning_restriction_id = ?", 
			...
			if (count($existingIssue) == 0) {
				$this->query("INSERT INTO warning (warning_last_update, warning_active, warning_layer_id, warning_issue_type, warning_x, warning_y, warning_source_plan_id, warning_restriction_id)
				...
				$this->query("UPDATE warning SET warning_last_update = ?, warning_active = 1 WHERE warning_id = ?", array(microtime(true), $existingIssue[0]["warning_id"]));
				if (count($existingIssue) > 1) {
					...
					for ($i = 1; $i < count($existingIssue); ++$i) {
						$this->query("UPDATE warning SET warning_last_update = ?, warning_active = 0 WHERE warning_id = ?", array(microtime(true), $existingIssue[$i]["warning_id"]));
					}
				}
			}
		}
		...
		foreach($removed as $removedIssue) {
			$this->query("UPDATE warning SET warning_active = 0, warning_last_update = ? WHERE warning_id = ?", array(microtime(true), $removedIssue['issue_database_id']));
		}
	}

	public function RemoveAllWarningsForLayer($layerId)
	{
		$this->query("UPDATE warning SET warning_active = 0, warning_last_update = ? WHERE warning_source_plan_id = ?", array(microtime(true), $layerId));
	}
	...
	{
		$this->query("UPDATE shipping_warning SET shipping_warning_active = 0, shipping_warning_lastupdate = ? WHERE shipping_warning_active = 1", array(microtime(true)));

		$newIssues = json_decode($issues, true);
		foreach($newIssues as $issue)
		{
			$this->query("INSERT INTO shipping_warning (shipping_warning_lastupdate, shipping_warning_source_geometry_persistent_id, shipping_warning_destination_geometry_persistent_id, shipping_warning_message) 
				VALUES(?, ?, ?, ?)
	...
	public function Latest($time){
		$result = array();
		$result['plan_issues'] = $this->query("SELECT 
		...
		$result['shipping_issues'] = $this->query("SELECT 
