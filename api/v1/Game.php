...
class Game extends Base
{
     ...
     * @return array{restrictions: array, plans: array, dependencies: array, CEL: ?array, REL: ?array, SEL: ?array{heatmap_settings: array, shipping_lane_point_merge_distance: int, shipping_lane_subdivide_distance: int, shipping_lane_implicit_distance_limit: int, maintenance_destinations: array, output_configuration: array}, MEL: ?array{x_min: int, x_max: int, y_min: int, y_max: int, cellsize: int, columns: int, rows: int, fishing_policy_settings: array}, meta: array, expertise_definitions: array, oceanview: array, objectives: array, events: array, region: string, edition_name: string, edition_colour: string, edition_letter: string, start: int, end: int, era_total_months: int, era_planning_months: int, era_planning_realtime: int, countries: string, minzoom: int, maxzoom: int, user_admin_name: string, user_region_manager_name: string, user_admin_color: string, user_region_manager_color: string, team_info_base_url: string, region_base_url: string, restriction_point_size: int, wiki_base_url: string, windfarm_data_api_url: ?string}|array{application_versions: array{client_build_date_min: string, client_build_date_max: string}}
     */
    // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
    ...
    public function ImportEvents(): void
    {
        ...
        foreach ($config['events'] as $event) {
            if ($event['country_id'] != -1) {
                $countries[] = $event['country_id'];
            } else {
                $countries[] = 1;
                $countries[] = 2;
                foreach ($this->GetCountries() as $value) {
            ...
            foreach ($countries as $country) {
                $this->getDatabase()->query(
                    ...
                    INSERT INTO game_event (
                        event_title, event_body, event_country_id, event_game_month, event_datetime, event_lastupdate
                    ) VALUES (?, ?, ?, -1, NULL, 100)
                    ",
                    array($event['title'], $event['body'], $country),
                    true
                );
            }
        }
