...
class Plan extends Base
{
     ...
     */
    // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
    public function setupPlans($dataModel): array|bool
    {
        if (!isset($dataModel['plans'])) {
            return false;
        }
     ...
     */
    // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
    public function setupRestrictions(array $dataModel): array|bool
    {
        $restrictions = $dataModel['restrictions'];
        foreach ($restrictions as $restrictionObj) {
            foreach ($restrictionObj as $restriction) {
                $qb = $this->getAsyncDatabase()->createQueryBuilder();
                $layerStart = await(
                    $this->getAsyncDatabase()->query(
                        $qb->select('layer_id')
                            ->from('layer')
                            ->where('layer_name = '.$qb->createPositionalParameter($restriction['startlayer']))
                    )->then(function (Result $result) {
                            return $result->fetchFirstRow();
                    })
                );
                if (empty($layerStart)) {
                    $warnings[] = $restriction['startlayer'].
                        " does not exist in this config file. Is it added in the layer meta?";
                    continue;
                }
                ...
                $layerEnd = await(
                    $this->getAsyncDatabase()->query(
                        $qb->select('layer_id')
                            ->from('layer')
                            ->where('layer_name = '.$qb->createPositionalParameter($restriction['endlayer']))
                    )->then(function (Result $result) {
                        return $result->fetchFirstRow();
                    })
                );
                if (empty($layerEnd)) {
                    $warnings[] = $restriction['endlayer'] .
                        " does not exist in this config file. Is it added in the layer meta?";
                    continue;
                }
                ...
                await(
                    $this->getAsyncDatabase()->query(
                        $qb->insert('restriction')
                           ->values([
                               'restriction_start_layer_id' => $qb->createPositionalParameter($layerStart['layer_id']),
                               'restriction_start_layer_type' =>
                                   $qb->createPositionalParameter($restriction['starttype']),
                               'restriction_sort' => $qb->createPositionalParameter($restriction['sort']),
                               'restriction_value' => $qb->createPositionalParameter($restriction['value']),
                               'restriction_type' => $qb->createPositionalParameter($restriction['type']),
                               'restriction_message' => $qb->createPositionalParameter($restriction['message']),
                               'restriction_end_layer_id' => $qb->createPositionalParameter($layerEnd['layer_id']),
                               'restriction_end_layer_type' => $qb->createPositionalParameter($restriction['endtype'])
                           ])
                    )->then(function (Result $result) {
        ...
        //Create a type unavailable for all layer types that have an available from date.
        foreach ($dataModel["meta"] as $layerId => $layerMeta) {
            foreach ($layerMeta["layer_type"] as $typeId => $typeMeta) {
                if (isset($typeMeta["availability"]) && (int)$typeMeta["availability"] > 0) {
                    $qb = $this->getAsyncDatabase()->createQueryBuilder();
                    await(
                        $this->getAsyncDatabase()->query(
                            $qb->insert('restriction')
                                ->values([
                                    'restriction_start_layer_id' => $qb->createPositionalParameter($layerId),
                                    'restriction_start_layer_type' => $qb->createPositionalParameter($typeId),
                                    'restriction_sort' => 'TYPE_UNAVAILABLE',
                                    'restriction_value' => 0,
                                    'restriction_type' => 'ERROR',
                                    'restriction_message' => 'Type not yet available at the plan implementation time.',
                                    'restriction_end_layer_id' => $qb->createPositionalParameter($layerId),
                                    'restriction_end_layer_type' => $qb->createPositionalParameter($typeId)
                                ])
                        )->then(function (Result $result) {
                            return $result->getAffectedRows();
                        })
