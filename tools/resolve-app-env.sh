...
# Export all .env variables as environment variables, e.g. APP_ENV
if [[ -z "${APP_ENV:-}" ]]; then
  ...
  if [[ ! -f "${DOTENV_FILE}" ]]; then
    echo "Could not find file ${DOTENV_FILE}" >&2
    pwd >&2
    exit 1
  fi
...
case "${APP_ENV:-}" in
  prod|dev)
    ...
    ;;
  *)
    echo "Encountered invalid environment: ${APP_ENV:-}" >&2
    ;;
esac
