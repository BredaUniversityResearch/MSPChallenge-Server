...
# dcu + production (p)
alias dcup="switch_project ${COMPOSE_PROJECT_NAME_PROD} && ede && ([[ \"${APP_ENV:-}\" == \"prod\" ]] || (echo \"Could not find APP_ENV=prod in dotenv\" && return 1 2>/dev/null || exit 1)) && $PRE_DCU && ${DCU_BASE} -f docker-compose.yml -f docker-compose.prod.yml up -d --remove-orphans"
...
# dl + simulations (s)
alias dls="[[ -n \"\${COMPOSE_PROJECT_NAME:-}\" ]] && ${ALIAS_DL_BASE} \${COMPOSE_PROJECT_NAME}-simulations-1"

# de = docker(d) execute(e) with container php
ALIAS_DE_BASE='MSYS_NO_PATHCONV=1 docker exec'
alias de="[[ -n \"\${COMPOSE_PROJECT_NAME:-}\" ]] && ${ALIAS_DE_BASE} \${COMPOSE_PROJECT_NAME}-php-1"
...
# de + tail log (tl)
alias detl='f() { ([[ "$1" != "" ]] || (echo "Please specify one of these files names:" && dells && return 1)) && (echo "press Ctrl+C to exit tail log"; de sh -c "tail -f /var/log/supervisor/$1" ; detl_finish); }; f'
...
MY2_SETUP="ede && (([ ! -f ./docker/database/my2_80.sql ] && echo 'Error: ./docker/database/my2_80.sql not found.' && return 1 2>/dev/null || exit 1) || read -p 'Please change the my2 user password in ./docker/database/my2_80.sql if needed. Press enter to continue.') && docker compose up -d database"
alias drg="[[ -n \"\${COMPOSE_PROJECT_NAME:-}\" ]] && ${MY2_SETUP} && docker stop ${COMPOSE_PROJECT_NAME}-grafana-1 ; docker rm ${COMPOSE_PROJECT_NAME}-grafana-1 ; MSYS_NO_PATHCONV=1 docker run -d -p 3000:3000 --name ${COMPOSE_PROJECT_NAME}-grafana-1 grafana/grafana"
