...
ALTER TABLE status
 ADD unique KEY IF NOT EXISTS idx01 (VARIABLE_NAME,timest);
...
ALTER TABLE current
 ADD unique KEY IF NOT EXISTS idx02 (VARIABLE_NAME);
...
-- Delta values
  ...
  replace into my2.status(variable_name,variable_value,timest) 
...
create user IF NOT EXISTS my2@'%' identified by 'euK@h89j6F5V';
