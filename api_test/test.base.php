...
class TestBase
{
        ...
        foreach ($methods as $method) {
            ...
            if (strstr($comment, "@TestMethod")) {
                try {
                    ...
                    print(
                        "❌ ". $type->getName()."::".$method->getName()." Failed. Exception thrown: ".$e->getMessage().
    ...
    protected function RunSubTask(ReflectionMethod $function, string $subtaskIdentifier, array $arguments)
    {
        try {
            ...
            print(
                "\t✅ ".$function->getDeclaringClass()->getName()."::".$function->getName().": $subtaskIdentifier".
                PHP_EOL
            );
        } catch (\Throwable $e) {
            ...
            print(
                ...
                ": $subtaskIdentifier Failed. Exception thrown: ".$e->getMessage(). " in ".$e->getFile().":".
                $e->getLine().PHP_EOL
            );
        }
        ob_flush();
    ...
    protected function DoRequest(string $endpoint, array $postData)
    {
        ...
        if ($response === false) {
            throw new Exception(
                "DoRequest to ".$endpoint." (".var_export($postData, true).") failed with error: ".curl_error($ch)
            );
        }
        curl_close($ch);
        return $response;
    ...
    protected function DoApiRequest(string $endpoint, array $postData)
    {
        ...
        if ($decodedJson === null) {
            throw new Exception(
                "DoApiRequest ".$endpoint." (".var_export($postData, true).") failed to decode JSON: ".
                json_last_error_msg().PHP_EOL."Response: ".$response
            );
        }

        if ($decodedJson["success"] == 0) {
            throw new Exception(
                "DoApiRequest ".$endpoint." (".var_export($postData, true).") Returned failure: ".
                $decodedJson["message"]
            );
        }
