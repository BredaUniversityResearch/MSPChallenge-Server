...
function configVersionsListToTable(configVersionsList)
{
    ...
    $.each(configVersionsList, function (i, v) {
        ...
        $.each(v.all_versions, function (n_i, n_v) {
            if (v.all_versions.length > 1 && n_i > 0) {
                childClass = colouring + " removeborder collapse allConfigVersions_"+v.filename;
            } else {
                childClass = colouring;
            }
            html += '<tr class="'+childClass+'">';
            if (v.all_versions.length > 1 && n_i == 0) {
                html += '<td style="min-width: 25px;" data-toggle="collapse" class="collapsed" data-target=".allConfigVersions_'+v.filename+'"><i class="fa"></i></td>';
            } else {
                html += '<td style="min-width: 25px;"><i class="fa"></i></td>';
            }
            if (n_i == 0) {
