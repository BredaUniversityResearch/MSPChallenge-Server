...
class GeoServer extends Base
{
    ...
    public function retrievePublic()
    {
        ...
        if (!$authoriser_call["success"]) throw new ServerManagerAPIException("Could not obtain public MSP Challenge GeoServer credentials.");
    ...
    public function get() 
    {
        if (empty($this->id)) throw new ServerManagerAPIException("Cannot obtain data without a valid geoserver id.");
        
        if (!$this->_db->query("SELECT * FROM game_geoservers WHERE id = ?", array($this->id))) throw new ServerManagerAPIException($this->_db->errorString());
        if ($this->_db->count() == 0) throw new ServerManagerAPIException("GeoServer not found.");
    ...
    {
        if (!$this->_db->query("SELECT id, name, address, available FROM game_geoservers")) throw new ServerManagerAPIException($this->_db->errorString());
    ...
    public function add() 
    {
        ...
        if (!$this->_db->query("INSERT INTO game_geoservers (
                                    ...
                                    $args)) throw new ServerManagerAPIException($this->_db->errorString());
